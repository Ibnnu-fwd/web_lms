<x-app-layout>
    <x-breadcrumb name="institution-dashboard" />
    <div class="text-gray-500 md:grid xl:grid-cols-2 2xl:grid-cols-3 gap-4 space-y-5 md:space-y-0">
        @foreach ($detailTransaction as $data)
            <x-card-container>
                ...
                @if ($data && $data->course && $data->course->id && $data->course->courseChapter->isNotEmpty())
                    <x-link-button title="Pelajari Sekarang" class="mt-6" color="dark"
                        route="{{ route('institution.course.detail', [$data->course->id, $data->course->courseChapter->first()->id]) }}" />
                @else
